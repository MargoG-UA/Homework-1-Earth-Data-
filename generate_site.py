...
merged_count = pd.merge(daily_alerts.nlargest(10, 'alert_count'), daily_aqi, on='date', how='left').fillna(-1)
merged_duration = pd.merge(daily_alerts.nlargest(10, 'total_duration'), daily_aqi, on='date', how='left').fillna(-1)

def format_aqi(val): return f"{val:.1f}" if val != -1 else "Немає даних"
...
<head>
    ...
    <title>Дашборд: Тривоги та Забруднення</title>
    <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
    <style>
        :root {{ --bg-color: #0f111a; --panel-bg: #1e2130; --text-main: #ffffff; --text-muted: #8b92a5; }}
        body {{ margin: 0; padding: 30px; background-color: var(--bg-color); color: var(--text-main); font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }}
        ...
        .card h3 {{ margin: 0 0 15px; font-size: 14px; text-transform: uppercase; color: #ffffff; }}
        .card .value {{ font-size: 32px; font-weight: bold; color: #ffffff; }}
        .grid-2 {{ grid-template-columns: repeat(2, 1fr); }}
        .grid-3 {{ grid-template-columns: repeat(3, 1fr); }}
        table {{ width: 100%; border-collapse: collapse; margin-top: 10px; }}
        th, td {{ text-align: left; padding: 12px; border-bottom: 1px solid rgba(255,255,255,0.05); color: #ffffff; }}
        th {{ text-transform: uppercase; font-size: 12px; font-weight: bold; background-color: #2a2e42; }}
    </style>
</head>
<body>
    <div class="header"><h1>Дашборд: Аналітика Повітряних Тривог та Якості Повітря</h1></div>
    <div class="grid grid-3">
        <div class="card"><h3>Середній AQI до (17-23 лют 2022)</h3><div class="value">{aqi_pre:.1f}</div></div>
        <div class="card"><h3>Середній AQI після (24 лют-2 бер 2022)</h3><div class="value">{aqi_post:.1f}</div></div>
        <div class="card"><h3>Різниця AQI</h3><div class="value">+{aqi_post - aqi_pre:.1f}</div></div>
    </div>
    <div class="grid grid-2">
        <div class="card"><h3>Середня тривалість тривог за рік (хв)</h3><canvas id="barChart"></canvas></div>
        <div class="card"><h3>Розподіл рекордних днів</h3><canvas id="pieChart"></canvas></div>
    </div>
    <div class="grid grid-2">
        <div class="card"><h3>Дні з найбільшою к-тю тривог</h3>
            <table><tr><th>Дата</th><th>К-ть тривог</th><th>Тривалість (хв)</th><th>AQI</th></tr>{merged_count_rows}</table>
        </div>
        <div class="card"><h3>Дні з найдовшими тривогами</h3>
            <table><tr><th>Дата</th><th>Тривалість (хв)</th><th>К-ть тривог</th><th>AQI</th></tr>{merged_duration_rows}</table>
        </div>
    </div>
<script>
    ...
    new Chart(document.getElementById('barChart'), {{
        type: 'bar',
        data: {{ labels: {list(avg_duration_yr.index)}, datasets: [{{ label: 'Тривалість (хв)', data: {list(avg_duration_yr.values)}, backgroundColor: '#3b82f6' }}] }},
        options: {{ scales: {{ y: {{ ticks: {{ color: '#ffffff' }} }}, x: {{ ticks: {{ color: '#ffffff' }} }} }} }}
    }});
    new Chart(document.getElementById('pieChart'), {{
        type: 'doughnut',
        data: {{ labels: ['Найдовші', 'Найбільше', 'Обидві', 'Інші'], datasets: [{{ data: [{len(days_only_longest)}, {len(days_only_most)}, {len(days_both)}, {days_other}], backgroundColor: ['#ef4444', '#f59e0b', '#10b981', '#374151'], borderWidth: 0 }}] }},
...
with open('index.html', 'w', encoding='utf-8') as f:
    f.write(html)
print("Готово! Файл index.html створено.")